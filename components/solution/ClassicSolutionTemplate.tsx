...
export default function SolutionTemplate({
    ...
    return (
        <div className="min-h-screen bg-white" ref={containerRef}>
            ...
            <div className="sticky top-[105px] xl:top-[110px] z-40 flex justify-center py-4 pointer-events-none">
            ...
            <section className="py-24 bg-gray-50">
                <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20">
                    <p className="mb-8 text-sm leading-7 text-gray-600">{language === 'KO' ? '아래는 선택 가능한 구성의 예시입니다. 지원 기능과 비용은 적용 장비와 설치 조건을 확인한 뒤 안내합니다.' : 'These are configuration examples. Features and costs depend on the selected equipment and installation conditions.'}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                        {features.map((feature, index) => (
                            ...
                            >
                                <div className="relative h-64 overflow-hidden cursor-pointer" role="button" tabIndex={0} aria-label={`${feature.title} 자세히 보기`} onClick={() => openModal(feature)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openModal(feature); } }}>
                                    {feature.image ? (
                                        <Image
                                            src={feature.image}
                                            alt={feature.title}
