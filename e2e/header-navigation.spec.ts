...
test.describe('Header Navigation', () => {
  test('should have "주문제작" menu item and navigate to /customize', async ({ page }) => {
    await page.goto('/');
    
    const customizeMenu = page.getByText('주문제작');
  ...
  test('should have "Customize" menu item in English mode', async ({ page }) => {
    await page.goto('/');
    
    const enButton = page.getByText('EN').first();
    await enButton.click();
    
    const customizeMenu = page.getByText('Customize');
    await expect(customizeMenu).toBeVisible();
  ...
  test('should have active highlight bar on menu hover', async ({ page }) => {
    ...
    const customizeMenu = page.getByText('주문제작').first();
  ...
  test('mobile menu should have "주문제작" item', async ({ page }) => {
    ...
    const customizeMenu = page.getByRole('link', { name: '주문제작' });
    await expect(customizeMenu).toBeVisible();
